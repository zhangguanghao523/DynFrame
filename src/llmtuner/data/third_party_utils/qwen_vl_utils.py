...
# 🔥🔥🔥 特殊调试打印：模块被导入 🔥🔥🔥
print(f"🚀🚀🚀 [DEBUG] qwen_vl_utils.py被导入! FORCE_QWENVL_VIDEO_READER={FORCE_QWENVL_VIDEO_READER} 🚀🚀🚀", file=sys.stderr)


@lru_cache(maxsize=1)
def get_video_reader_backend() -> str:
    # 🔥🔥🔥 特殊调试打印：函数被调用 🔥🔥🔥
    print(f"🎯🎯🎯 [DEBUG] get_video_reader_backend()被调用! 🎯🎯🎯", file=sys.stderr)

    if FORCE_QWENVL_VIDEO_READER is not None:
        video_reader_backend = FORCE_QWENVL_VIDEO_READER
        print(f"🔧🔧🔧 [DEBUG] 使用强制指定的后端: {video_reader_backend} 🔧🔧🔧", file=sys.stderr)
    elif is_torchcodec_available():
        video_reader_backend = "torchcodec"
        print(f"📹📹📹 [DEBUG] 自动选择torchcodec后端 📹📹📹", file=sys.stderr)
    ...
    print(f"✅✅✅ [DEBUG] get_video_reader_backend()返回: {video_reader_backend} ✅✅✅", file=sys.stderr)
...
def fetch_video(ele: Dict[str, Any], image_patch_size: int = 14, return_video_sample_fps: bool = False,
    ...
    # 🔥🔥🔥 特殊调试打印：fetch_video被调用 🔥🔥🔥
    print(f"🎥🎥🎥 [DEBUG] fetch_video()被调用! 视频路径: {ele.get('video', 'unknown')} 🎥🎥🎥", file=sys.stderr)
    ...
    if isinstance(ele["video"], str):
        print(f"📁📁📁 [DEBUG] 处理视频文件路径，调用get_video_reader_backend() 📁📁📁", file=sys.stderr)
        video_reader_backend = get_video_reader_backend()
        print(f"⚙️⚙️⚙️ [DEBUG] 使用后端 {video_reader_backend} 读取视频 ⚙️⚙️⚙️", file=sys.stderr)
