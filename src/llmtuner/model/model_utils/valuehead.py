...
def prepare_valuehead_model(model: "PreTrainedModel") -> None:
    if getattr(model.config, "model_type", None) in ["llava", "vipllava"]:
    ...
    if getattr(model.config, "model_type", None) == "internlm2":
        ...
        setattr(model, "_keys_to_ignore_on_save", ["lm_head.weight"])
    if getattr(model.config, "model_type", None) == "internvl_chat" and \
            model.config.llm_config.architectures[0] == 'InternLM2ForCausalLM':
        setattr(model, "lm_head", model.language_model.output)
        setattr(model, "_keys_to_ignore_on_save", ["lm_head.weight"])
